...
import { usePagination, useProductRequisitionByCreator, useUser } from '@/hooks'
...
const ProductRequisitionsEmployee: React.FC = () => {
  ...
  const filteredData = useMemo(() => {
    if (!data?.result?.items || data.result.items.length === 0 || !user) return []

    return data.result.items.filter((item) => item.creatorSlug === user.result.slug)
  }, [data?.result?.items, user])
